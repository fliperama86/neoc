...
neoc_platform_t *platform_create(const char *title, int width, int height) {
    ...
    p->framebuffer = SDL_CreateTexture(p->renderer,
...
void platform_destroy(neoc_platform_t *p) {
    ...
    if (p->framebuffer) SDL_DestroyTexture(p->framebuffer);
        ...
        switch (event.type) {
            ...
            case SDL_KEYUP: {
                ...
                switch (event.key.keysym.scancode) {
                    case SDL_SCANCODE_UP:    btn = NEOC_BTN_UP; break;
                    case SDL_SCANCODE_DOWN:  btn = NEOC_BTN_DOWN; break;
                    case SDL_SCANCODE_LEFT:  btn = NEOC_BTN_LEFT; break;
                    case SDL_SCANCODE_RIGHT: btn = NEOC_BTN_RIGHT; break;
                    case SDL_SCANCODE_Z:     btn = NEOC_BTN_A; break;
                    case SDL_SCANCODE_X:     btn = NEOC_BTN_B; break;
                    case SDL_SCANCODE_C:     btn = NEOC_BTN_C; break;
                    case SDL_SCANCODE_V:     btn = NEOC_BTN_D; break;
                    case SDL_SCANCODE_RETURN: btn = NEOC_BTN_START; break;
                    case SDL_SCANCODE_RSHIFT: btn = NEOC_BTN_SELECT; break;
                    case SDL_SCANCODE_ESCAPE:
                        if (pressed) p->running = false;
                        break;
                    default: break;
                }

                if (btn && input) {
                    if (pressed) input_press(input, 0, btn);
                    else input_release(input, 0, btn);
                }
                break;
            }
        }
...
void platform_render_frame(neoc_platform_t *p, const uint32_t *pixels, int width, int height) {
    ...
    SDL_UpdateTexture(p->framebuffer, NULL, pixels, width * sizeof(uint32_t));
...
void platform_render_frame(neoc_platform_t *p, const uint32_t *pixels, int width, int height) {
    (void)p; (void)pixels; (void)width; (void)height;
}

void platform_audio_init(neoc_platform_t *p, int sample_rate, platform_audio_callback_t callback, void *userdata) {
    (void)p; (void)sample_rate; (void)callback; (void)userdata;
}
